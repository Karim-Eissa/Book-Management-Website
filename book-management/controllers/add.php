...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    } elseif (trim(strlen($_POST['title'])) > 50) {
        $errors['title'] = 'Title must be less than 50 characters';
    }

    if (empty(trim($_POST['author']))) {
        $errors['author'] = 'Author is required';
    } elseif (trim(strlen($_POST['author'])) > 50) {
    ...
    } elseif (trim(strlen($_POST['summary'])) > 1000) {
    ...
    if (empty($errors)) {
        ...
        $query = 'INSERT INTO books (title, author, publishing_date, cover_image, summary) VALUES (:title, :author, :publishing_date, :cover_image, :summary)';
        $params = [
            'title' => $title,
            'author' => $author,
